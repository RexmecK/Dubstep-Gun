...
function arms:init()
	...
	local createTransform = function ()
		...
		transforms:add("arms_global",sb.jsonMerge({position = armCenter, rotation = 0, rotationPoint = {0,0}}, animationTranformationGroup.arms_global.transform or {}),
			function(name,thisTransform, dt) 
				if animator.hasTransformationGroup(name) then --Check to prevent crashing
					local setting  = transforms.calculateTransform({
...
function arms:update(dt)
	...
	if self.curdirection ~= self.direction then

		self.curdirection = self.direction

		local handhold = activeItem.hand()

		if not self.twohand then --non two hand here
			animator.setAnimationState("left", self:whatlayer(self.direction, handhold))
			animator.setAnimationState("right", self:whatlayer(-self.direction, handhold))
